...
Фреймворком для сайта был выбран Django. Питон-язык, которым я владею лучше всего, Django удобен в использование и универсален. Так что проект можно назвать full-python. База данных использованная в проекте - SqLite3. Для создания динамических форм был использован формат JSON. Поскольку по-стандарту SqLite3 не поддерживает этот формат, то для работы с базой данных следует заменить sqllte.dll в корневой папке python на файл прикрепленный к проекту.
...
Директория этой части проекта сильно больше 100 мб.Ссылка на ЯндексДиск : https://disk.yandex.ru/d/dM3hmhMHZVtlIg
